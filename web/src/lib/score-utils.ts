import type { Scholarship } from '@/features/bourses/types'
...
export function calculateScoreBreakdown(scholarship: Scholarship): ScoreBreakdown {
    const now = new Date()
    const verified = new Date(scholarship.lastVerifiedAt)
    ...
    // Reliability
    const domain = scholarship.sourceUrl.match(/https?:\/\/([^/]+)/)?.[1] || ''
    ...
    let stability = scholarship.healthScore - freshness - reliability

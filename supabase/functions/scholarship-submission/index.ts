...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { name, source_url, amount_min, amount_max, deadline_at, ...rest } = body

    if (!name || !source_url) {
        return errorResponse(400, 'Bad Request', 'name and source_url are required')
    }
    ...
    // 4. DB Insertion (Using Service Role to enforce SUBMITTED status)
